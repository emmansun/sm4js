...
test('test sample 1', (t) => {
  ...
  const c = new SM4(key)
  const out = c.encrypt(key)
  t.equal(sjcl.codec.hex.fromBits(out), expected)
  t.end()
})

test('test sample 2', (t) => {
  let out = sjcl.codec.hex.toBits('0123456789abcdeffedcba9876543210')
  const expected = 'd735e91cc5689cf312bcc1efb740e813'
  const c = new SM4(out)
...
test('test sm4-gcm', (t) => {
  ...
  for (const c of cases) {
    const sm4 = new SM4(sjcl.codec.hex.toBits(c.key))
...
test('test sm4-cbc', (t) => {
  ...
  for (const c of cases) {
    const sm4 = new SM4(sjcl.codec.hex.toBits(c.key))
    const got = sjcl.mode.cbc.encrypt(
      sm4,
      sjcl.codec.hex.toBits(c.plaintext),
      sjcl.codec.hex.toBits(c.iv)
    )
    t.equal(sjcl.codec.hex.fromBits(got), c.ciphertext)
  }
  t.end()
